    ...
    IEndDragHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    {
        if (this.transform.GetChild(1).GetComponent<CharacterClicked>().selected == false) return;
    ...
    {
        if (this.transform.GetChild(1).GetComponent<CharacterClicked>().selected == false) return;//未被选中不得执行
